...
class hv_account_register_payment(models.TransientModel):
    ...
    def _prepare_payment_vals(self, invoices):
        values = super(hv_account_register_payment,
    ...
    def create_payments(self):
        ...
        action_vals = super(hv_account_register_payment,
...
class hv_account_payment(models.Model):
    ...
    def create(self, vals):
        if self._context.get('batch_payment_id'):
            vals.update({'email_vendor': self.env['res.partner'].browse(
                vals.get('partner_id')).email})
        return super(hv_account_payment, self).create(vals)

    def preview_payments(self):
        default_template = self.env.ref(
            'hv_send_remittance_advice.email_template_batch_payment', False)
        ...
        ctx = dict(
            ...
            default_template_id=default_template and
            default_template.id or False,
            default_composition_mode='comment',
            force_email=True,
        )
        message_ref = self.env.ref('mail.email_compose_message_wizard_form')
        msg_view_id = message_ref and message_ref.id or False
        return {
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'mail.compose.message',
            'views': [(msg_view_id, 'form')],
            'view_id': msg_view_id,
            'target': 'new',
...
class hv_batch_email_send_abs(models.AbstractModel):
    ...
    _description = 'Batch Payment Email Send Abstract'
...
class hv_account_batch_payment(models.Model):
    ...
    payment_ids = fields.One2many('account.payment', 'batch_payment_id',
                                  string="Payments",
        ...
        for record in self:
            if record.payment_ids:
                ...
                if len(all_companies) > 1:
                    raise ValidationError(_("All payments in the batch must"
                                            " belong to the same company."))
                all_journals = set(record.payment_ids.mapped('journal_id'))
                if len(all_journals) > 1 or \
                        record.payment_ids[0].journal_id != record.journal_id:
                    raise ValidationError(_("The journal of the batch payment"
                                            " and of the payments it contains"
                                            " must be the same."))
                ...
                if all_types and record.batch_type not in all_types:
                    raise ValidationError(_("The batch must have the same type"
                                            " as the payments it contains."))
                all_payment_methods = set(
                    record.payment_ids.mapped('payment_method_id'))
                if len(all_payment_methods) > 1:
                    raise ValidationError(_("All payments in the batch must"
                                            " share the same payment method."))
                if all_payment_methods and \
                        record.payment_method_id not in all_payment_methods:
                    raise ValidationError(_("The batch must have the same"
                                            " payment method as the payments"
                                            " it contains."))
    ...
    def action_send_remittance_advice(self):
        ...
        for item in self.payment_ids:
            if not vendor.get(item.partner_id.id):
                ...
                vendor[item.partner_id.id]['payment_ids'].append(item.id)
        if not vendor:
            return self.env['havi.message'].\
                action_warning('Payment to send not found.',
        ...
        for item in vendor:
            ...
            for pid in batch.payment_ids:
                pid.email_send += 1
        return self.env['havi.message'].\
            action_warning('Send remittance advice completed.',
                           'Send Remittance Advice')
    ...
    def cancel_payments(self):
        ...
        return self.env['havi.message'].\
            action_warning('Payments cancel is completed.',
                           'Payments Cancel')
