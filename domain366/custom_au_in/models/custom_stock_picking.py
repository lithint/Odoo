    ...
    def invoice_line_non_kit(self):
        ...
        for pick_line in self.move_lines:
            ...
            inv_line = invoice_line_obj.create({
                    ...
                    'price_unit': pick_line.price_unit *-1,
                    'quantity': pick_line.product_uom_qty,
                    'uom_id': pick_line.product_id.uom_id.id,
                    'product_id': pick_line.product_id.id,
                    })
            order_line_ids = sale_order_line_obj.search([('order_id', '=', sale_order.id),('product_id', '=',pick_line.product_id.id )])
    ...
    def get_account_properties(self):
        ...
        for pick_line in self.move_lines:
            if pick_line.product_id.property_account_income_id:
                ...
                ir_property_obj.search([('name', '=', 'property_account_income_categ_id')])
